$ (function(){
  ...
  socket.on('onlineStack',function(stack){
    ...
    for (var user in stack){
      ...
      if(user == username){
        var txt1 = $('<button class="btn btn-info  btn-md" style="background-color: transparent;"> </button>').text(user).css({"font-size":"18px"});
      }
      else{
        var txt1 = $('<button id="ubtn" class="btn btn-success  btn-md" style="background-color: transparent;">').text(user).css({"font-size":"18px"});
      }
    ...
    if(data.room == roomId){
      ...
      if(data.result.length != 0){
        ...
        for (var i = 0;i < data.result.length;i++) {
          ...
          var txt1 = $('<span></span>').text(data.result[i].msgFrom+" : ").css({"color":"rgb(2 175 239)"});
          var txt2 = $('<span></span>').text(chatDate).css({"float":"right","color":"#a6a6a6","font-size":"16px"});
          var txt3 = $('<p></p>').append(txt1,txt2);
          var txt4 = $('<p></p>').text(data.result[i].msg).css({"color":"white"});
  ...
  socket.on('chat-msg',function(data){
    ...
    var txt1 = $('<span></span>').text(data.msgFrom+" : ").css({"color":"rgb(2 175 239)"});
    var txt2 = $('<span></span>').text(chatDate).css({"float":"right","color":"#a6a6a6","font-size":"16px"});
    var txt3 = $('<p></p>').append(txt1,txt2);
    var txt4 = $('<p></p>').text(data.msg).css({"color":"white"});
